...
void loop_search(CMemoryReaderWriter *pRwDriver, uint64_t hProcess, size_t nWorkThreadCount) {
	...
	GetModuleDataAreaSection(pRwDriver, hProcess, targetModuleName, vDataList);
	...
	MultiThreadExecOnCpu(nWorkThreadCount,
		[pRwDriver, hProcess, splitSize, spWorkMemWrapper, execStartAddr, &vResultInfo, &resultMutex]
